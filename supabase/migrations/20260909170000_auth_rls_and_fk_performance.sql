...
CREATE POLICY project_nodes_owner_select ON public.project_nodes
  FOR SELECT TO authenticated USING (private.owns_project(project_id, (select auth.uid())));
...
CREATE POLICY project_nodes_owner_insert ON public.project_nodes
  FOR INSERT TO authenticated WITH CHECK (private.owns_project(project_id, (select auth.uid())));
...
CREATE POLICY project_nodes_owner_delete ON public.project_nodes
  FOR DELETE TO authenticated USING (private.owns_project(project_id, (select auth.uid())));
